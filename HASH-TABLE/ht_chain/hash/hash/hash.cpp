...
struct Data         // необходимые дынные об 1ом объекте 
{
    ...
    bool operator ==(const Data& data) const // перегрузка оператора для функции remove в функции HashTab::pop
    {
        return FIO == data.FIO and address == data.address and passport == data.passport;
    }
};
...
struct HashTab
{
    ...
    {
        this->size = size;
        mas = new list<Data>[this->size];
        
        for (int i = 0; i < size; i++)
            mas[i].push_front(Data());
    ...
    void pop(string key, const int size); // удаление элемента
...
{
    bool res = false;
    auto it = mas[index].begin(); // итератор, необходим для обращения к элементу списка
    if ((*it).FIO == "NULL" and (*it).address == "NULL" and (*it).passport == "NULL")
        res = true;
    return res;
}
...
void HashTab::printTable()
{
    for (int i = 0; i < size; i++)
    {
        ...
        for (int j = 0; j < mas[i].size(); j++)
        {
            ...
            cout << "ФИО: " << (*it).FIO << endl;
            cout << "Место проживания: " << (*it).address << endl;
            cout << "Номер паспорта: " << (*it).passport << endl;
            it++;
        }
...
void HashTab::insert(Data person, const int size)
{
    ...
    {
        mas[index].push_back(person);
       
    }
}

void HashTab::pop(string key, const int size) // удаление элемента
{
    ...
    if (mas[index].size() == 1)
    {
        ...
        if ((*it).address == key)
        {
            *it = Data(); // значение по умолчанию NULL
            return;
        }
        else
        {
    ...
    else
    {
        int i = 1;
        bool fl = false;
        while (i <= mas[index].size())
        {
            Data data = *it;
            if (data.address == key)
            {
                mas[index].remove(data);
                fl = true;
                break;
            }
            i++;
